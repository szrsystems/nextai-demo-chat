...
/* A modellnevet ellenorizzuk. Ha valaki veletlenul ide masolja az API kulcsot,
   a kulcs nem kerul bele a keresbe es a naplokba, hanem az alapertelmezes lep eletbe. */
function modellNev(ertek, alap) {
  const v = String(ertek || '').trim();
  if (/^gemini[a-z0-9.\-]{2,60}$/.test(v)) return v;
  if (v) {
    console.error('FIGYELEM: a megadott modellnev ervenytelen, ezert az alapertelmezes lep eletbe: ' + alap +
      '. Ha ide veletlenul API kulcsot masoltal, a kulcsot azonnal cserald le a Google AI Studioban.');
  }
  return alap;
}
const MODELL      = modellNev(process.env.GEMINI_MODEL, 'gemini-3.1-flash-lite');
...
const MODELL_2    = modellNev(process.env.GEMINI_MODEL_2, 'gemini-3.5-flash-lite');  /* tartalek, ha az elso terhelt */
